...
export const TenderDetail: React.FC = () => {
    ...
    const [tender, setTender] = useState<UploadedDocument | null>(null);
    ...
    useEffect(() => {
        const fetchBaseData = async () => {
            ...
            try {
                ...
                // If profiles exist, auto-select the first one for matching
    ...
    useEffect(() => {
        const fetchMatch = async () => {
            if (!id || !selectedProfileId || selectedProfileId === 'none') {
                setMatch(null);
                return;
            }
            setMatchLoading(true);
            try {
                // We need the vendor's document ID for matching.
                // For simplicity, we assume the user has at least one vendor doc.
                const { getMyDocuments } = await import('../services/documentService');
                const myDocs = await getMyDocuments('vendor');
                if (myDocs.length > 0) {
                    const result = await getMatchDetail(myDocs[0].id, id);
                    setMatch(result);
                }
            } catch (err) {
